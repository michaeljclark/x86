...
extern x86_opr_formats x86_opr_formats_intel_hex;
extern x86_opr_formats x86_opr_formats_intel_dec;
...
static uint x86_opr_ew_bytes(uint opr)
{
...
static uint x86_opr_ew_size(uint opr)
{
    switch(opr & x86_opr_ew_mask) {
    ...
    case x86_mod_disp0:
        if (q.rm != x86_rm_sp_sib && (q.b & 7) == x86_rm_bp_disp0) {
            if (x86_codec_is64(c) && disp) {
                ...
                len = snprintf(buf, buflen, fmt->ptr_rip,
                    x86_ptr_size_str(ptrsz));
            }
            else {
