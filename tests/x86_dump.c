...
static void x86_disasm(x86_ctx *ctx, uchar *insn, size_t insnlen, int verbose)
{
    x86_buffer buf;
    x86_codec codec;
    char str[128];
...
int main(int argc, char **argv)
{
    ...
    if (!help && !filename && i < argc) {
        ...
        fprintf(stderr, "usage: %s [-v] [-d] (-f <file.bin> | <hex> ...)\n",
            argv[0]);
        return -1;
    }
    else if (filename) {
    ...
    if (buf.data && buf.length) {
        ...
        x86_disasm(ctx, buf.data, buf.length, verbose);
        x86_ctx_destroy(ctx);
    }
